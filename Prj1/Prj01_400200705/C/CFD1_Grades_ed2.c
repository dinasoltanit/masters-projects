...
int main(void)
{
    ...
    */
    ...
    case 1:
        ...
        for (j = 0; j < 10; j++)
        {
            printf("%s %s   %f \n", M1_name[j][0], M1_name[j][1], M1_Grade[j]);
        }
        break;
    case 2:
        /* ******************************Case 2********************************** */
    ...
    printf("Do you want to print the results in a text file? (y/n)\n");
    scanf("%c", &ans);
    scanf("%c", &ans);
    printf("You entered %c.\n", ans);
    if (ans == 'y')
    {
        fptr = fopen("D:\\EDocuments\\Grades.txt", "a");
        if (Method == 1)
        {
            fprintf(fptr, "You have used method 1.\n");
            fprintf(fptr, "Name    Grade\n");
            for (j = 0; j < 10; j++)
            {
                fprintf(fptr, "%s %s   %f \n", M1_name[j][0], M1_name[j][1], M1_Grade[j]);
            }
        ...
        {
            fprintf(fptr, "You have used method 2.\n");
            fprintf(fptr, "Name    Grade\n");
            for (k = 0; k < 10; ++k)
            {
                M2_Grade[k] = M2_HW[k] * percents[0] + M2_P1[k] * percents[1] + M2_P2[k] * percents[2] + M2_P3[k] * percents[3] + M2_P4[k] * percents[4] + M2_mid[k] * percents[5] + M2_fin[k] * percents[6] + (4 - M2_Absences[k] - (M2_Delays[k] / 3));
        ...
        {
            fprintf(fptr, "You have used the default settings.\n");
            fprintf(fptr, "Name             Grade\n\n");
            for (k = 0; k < 10; k++)
            {
                M2_Grade[k] = M2_HW[k] * percents[0] + M2_P1[k] * percents[1] + M2_P2[k] * percents[2] + M2_P3[k] * percents[3] + M2_P4[k] * percents[4] + M2_mid[k] * percents[5] + M2_fin[k] * percents[6] + (4 - M2_Absences[k] - (M2_Delays[k] / 3));
