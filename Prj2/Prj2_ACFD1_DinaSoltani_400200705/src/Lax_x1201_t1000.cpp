...
//  
//  Equation: ut = -c * ux
//  Method: Finite difference, Lax Method, Explicit
//
...

{
  ...
  string data_filename = "Lax_x1201_t1000.txt";
  ofstream data_unit;
  double dt; // time-step
  double dx; // size-step
  int i;
  ...
  cout << "\n";
  cout << "Finite Difference: Lax Method for 1D Wave Equation:\n";
  cout << "  C++ language\n";
  cout << "\n";
  ...
  cout << "           = 0 elsewhere.\n";
  cout << "\n";
  cout << "  We use a method known as Lax:\n";
...
// ***** Core Calculation: begin *****
  for ( i = 0; i < nt; i++ )
  {
    ...
    for ( j = 0; j < nx; j++ )
    {
      ...
      if ( j != nx-1)
      {
          unew[j] = (0.5 * ( 1 - (c * dt / dx) ) ) * u[jp1] + (0.5 * ( 1 + (c * dt / dx) ) ) * u[jm1];
          //if ( u[j] < 1e-04 ) { u[j] = 0; }
      }
      else
      {
...
//
  cout << "\n";
  cout << "  Finite Difference: Lax for 1D Wave Equation\n";
